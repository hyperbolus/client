<template>
  <layout-standard title="Library">
    ...
    <h4 v-if="levels.length === 0 && loading">Loading...</h4>
    <h4 v-if="levels.length === 0 && !loading">No levels found</h4>
    <div v-else class="flex flex-col space-y-4">
      <level-ticket v-for="(level, index) in levels" v-bind:key="index" :title="level.title" :subtitle="level.artist">
        <div onclick="alert('Pretend this works :)')" class="click bg-neutral-200 dark:bg-neutral-800 hover:bg-neutral-300 dark:hover:bg-neutral-600 items-center p-2 mr-2 h-fit transition transition-colors text-neutral-700 dark:text-neutral-200">
...
export default {
  ...
  methods: {
    async refresh() {
      ...
      this.levels = [];
      let lines;
      await createDir('soundodger\\levels', {dir: 18, recursive: true});
      await readDir('soundodger\\levels', {dir: 18}).then((data) => lines = data);
